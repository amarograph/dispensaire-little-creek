      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 420px', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div style={{ ...panel, borderLeft: '4px solid #8B3A1A', padding: '28px 34px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: 8, right: 24, fontFamily: DISPLAY, fontSize: 80, color: 'rgba(200,147,42,0.05)', lineHeight: 1 }}>✚</div>

            <div>
              <div style={{ fontFamily: MONO, fontSize: 15, color: '#8B3A1A', letterSpacing: '0.16em', marginBottom: 12, textTransform: 'uppercase' }}>
                ✦ Dispensaire Medical · Territoire des Ameriques
              </div>
              <div style={{ fontFamily: DISPLAY, fontSize: 40, color: '#E8DCC8', lineHeight: 1.15, letterSpacing: '0.03em' }}>
                Carnet Medical <span style={{ color: '#C8932A' }}>RedM</span>
              </div>
              <div style={{ fontFamily: BODY, fontWeight: 400, fontSize: 19, color: '#8A7560', marginTop: 10, letterSpacing: '0.04em' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 22 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div>
                  ...
                  <div style={{ fontFamily: BODY, fontWeight: 600, fontSize: 13, color: '#4A3520', letterSpacing: '0.08em', textTransform: 'uppercase' }}>En service</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
            {[
              ...
              { label: 'ACTES REALISES', val: '14',  unit: '',  col: '#8B9E6A' },
              { label: 'TAUX SURVIE',    val: '85',  unit: '%', col: '#C87070' },
              { label: 'JOURS EN POSTE', val: '312', unit: '',  col: '#7A9AAA' },
            ].map(v => (
              <div key={v.label} style={{ ...panel, padding: '18px 20px', borderBottom: `3px solid ${v.col}50` }}>
                <div style={{ fontFamily: BODY, fontWeight: 700, fontSize: 13, color: '#4A3520', letterSpacing: '0.10em', marginBottom: 10, textTransform: 'uppercase' }}>{v.label}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontFamily: DISPLAY, fontSize: 16, color: '#C8932A', whiteSpace: 'nowrap', letterSpacing: '0.08em' }}>Modules du Dispensaire</span>
            <div style={{ flex: 1, height: 1, background: 'linear-gradient(90deg, rgba(200,147,42,0.4), transparent)' }} />
            <span style={{ fontFamily: BODY, fontWeight: 700, fontSize: 14, color: '#4A3520', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{MODULES.length} Services</span>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
            {MODULES.map(m => {
              ...
              return (
                ...
                >
                  <div style={{ position: 'absolute', bottom: 8, right: 12, fontFamily: DISPLAY, fontSize: 48, color: `${m.color}07`, lineHeight: 1, pointerEvents: 'none' }}>{m.ornament}</div>

                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ width: 64, height: 64, background: m.color + '12', border: `1px solid ${m.color + '45'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 30 }}>
                      {m.icon}
                    </div>
                    <div style={{ fontFamily: BODY, fontWeight: 700, fontSize: 14, padding: '5px 13px', background: m.color + '15', color: m.color, border: `1px solid ${m.color + '40'}`, letterSpacing: '0.10em', textTransform: 'uppercase' }}>
                      {m.badge}
                    </div>
                  </div>

                  <div>
                    <div style={{ fontFamily: DISPLAY, fontSize: 20, color: '#E8DCC8', lineHeight: 1.2, marginBottom: 6, letterSpacing: '0.03em' }}>{m.label}</div>
                    <div style={{ fontFamily: BODY, fontWeight: 700, fontSize: 13, color: m.color, letterSpacing: '0.12em', marginBottom: 10, textTransform: 'uppercase' }}>{m.sub}</div>
                    <div style={{ fontFamily: BODY, fontWeight: 400, fontSize: 15, color: '#8A7560', lineHeight: 1.65 }}>{m.desc}</div>
                  </div>

                  <div style={{ paddingTop: 12, borderTop: `1px solid ${m.color + '25'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontFamily: BODY, fontWeight: 700, fontSize: 13, color: '#2A1B0E', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Disponible</span>
                    <span style={{ fontFamily: MONO, fontSize: 15, color: h ? m.color : '#4A3520', transition: 'color 0.15s' }}>→ Consulter</span>
                  </div>
                </div>
              );
            })}
          ...
          <div onClick={() => router.push('/redm/certificats/urgence')} style={{ background: 'rgba(139,58,26,0.08)', border: '1px solid rgba(139,58,26,0.35)', borderLeft: '4px solid #8B3A1A', padding: '22px 32px', display: 'flex', alignItems: 'center', gap: 20, cursor: 'pointer' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: DISPLAY, fontSize: 22, color: '#C87070', letterSpacing: '0.04em' }}>Cas d'Urgence</div>
              <div style={{ fontFamily: BODY, fontWeight: 600, fontSize: 14, color: '#4A3520', letterSpacing: '0.08em', marginTop: 5, textTransform: 'uppercase' }}>Ouvrir immediatement un certificat d'urgence vitale</div>
            </div>
            <div style={{ fontFamily: MONO, fontSize: 17, color: '#8B3A1A' }}>→ Intervenir</div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              {ETAT.map(e => (
                <div key={e.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontFamily: BODY, fontWeight: 400, fontSize: 17, color: '#8A7560' }}>{e.label}</span>
            ...
            <div>
              {REGISTRE.map((l, i) => (
                <div key={i} style={{ padding: '14px 20px', borderBottom: i < REGISTRE.length - 1 ? '1px solid rgba(200,147,42,0.07)' : 'none', display: 'flex', alignItems: 'flex-start', gap: 13 }}>
                  ...
                  <div>
                    <div style={{ fontFamily: MONO, fontSize: 14, color: '#4A3520', letterSpacing: '0.07em', marginBottom: 4 }}>{l.heure}</div>
                    <div style={{ fontFamily: BODY, fontWeight: 400, fontSize: 16, color: '#8A7560', lineHeight: 1.5 }}>{l.msg}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ padding: '14px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              {PATIENTS.map(p => (
                <div key={p.nom} style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: BODY, fontWeight: 600, fontSize: 17, color: '#B8A888' }}>{p.nom}</div>
                  </div>
          ...
          <div style={{ ...panel, padding: '20px 22px', borderLeft: '3px solid rgba(200,147,42,0.30)' }}>
            <div style={{ fontFamily: BODY, fontWeight: 300, fontSize: 18, color: '#8A7560', lineHeight: 1.75, fontStyle: 'italic' }}>
