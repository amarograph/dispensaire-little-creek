...
const MODULES = [
  ...
  { id: 'agenda',       href: '/redm/agenda',        icon: '📅', label: 'Agenda',                 sub: 'RENDEZ-VOUS',       desc: 'Planifier et consulter les rendez-vous du dispensaire. RDV du cabinet anonymisés.', color: '#526C61', badge: 'AGD' },
  { id: 'cabinet',      href: '/redm/cabinet',       icon: '🛋', label: 'Cabinet Thérapeutique',  sub: "SOINS DE L'ÂME",   desc: "Suivi psychologique, consultations de l'esprit et thérapies de l'époque.",   color: '#49654D', badge: 'PSY' },
  { id: 'direction',    href: '/redm/direction',     icon: '🏛', label: 'Direction',              sub: 'ACCÈS RÉSERVÉ',     desc: 'Comptabilité et gestion administrative du dispensaire — direction & co-direction.', color: '#80682D', badge: 'DIR' },
];
...
const CARD_BG   = 'rgba(247,237,215,0.92)';
const CARD_BG_H = 'rgba(247,237,215,0.97)';

const PANEL: React.CSSProperties = {
  background: 'rgba(247,237,215,0.92)',
  border: '1px solid rgba(142,122,74,0.35)',
};
const PANEL_HEAD: React.CSSProperties = {
  padding: '14px 22px',
  borderBottom: '1px solid rgba(142,122,74,0.22)',
  fontFamily: DISPLAY,
  fontSize: 18,
  color: '#254B50',
  letterSpacing: '0.10em',
  background: 'rgba(142,122,74,0.08)',
    ...
    <div style={{ fontFamily: BODY }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 420px', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {/* ── EN-TÊTE ── */}
          <div style={{ ...PANEL, borderLeft: '4px solid #A18B52', padding: '28px 34px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: 10, right: 28, fontFamily: DISPLAY, fontSize: 130, color: 'rgba(142,122,74,0.06)', lineHeight: 1, pointerEvents: 'none', userSelect: 'none' }}>✚</div>

            <div>
              <div style={{ fontFamily: MONO, fontSize: 17, color: '#64684E', letterSpacing: '0.16em', marginBottom: 12, textTransform: 'uppercase' }}>
                ✦ {TITRES.surtitle}
              </div>
              <div style={{ fontFamily: DISPLAY, fontSize: 50, color: '#183746', lineHeight: 1.15 }}>
                {TITRES.title}
              </div>
              <div style={{ fontFamily: BODY, fontWeight: 400, fontSize: 24, color: '#536052', marginTop: 10, letterSpacing: '0.04em' }}>
                {TITRES.subtitle}
              </div>
            </div>

            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 22, paddingTop: 18, borderTop: '1px solid rgba(142,122,74,0.20)' }}>
              <div style={{ fontFamily: MONO, fontSize: 18, color: '#536052', textTransform: 'capitalize' }}>{date}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
                <div style={{ fontFamily: MONO, fontSize: 47, color: '#244958', lineHeight: 1, letterSpacing: '0.06em' }}>{time}</div>
                <div>
                  <div style={{ fontFamily: MONO, fontSize: 17, color: '#42663C', letterSpacing: '0.10em', textTransform: 'uppercase' }}>✦ Ouvert</div>
                  <div style={{ fontFamily: BODY, fontWeight: 600, fontSize: 16, color: '#596451', letterSpacing: '0.08em', textTransform: 'uppercase' }}>En service</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div
            ...
            style={{ ...PANEL, borderLeft: '4px solid #80682D', padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 18, cursor: 'pointer', transition: 'background 0.15s' }}
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: DISPLAY, fontSize: 19, color: '#80682D', lineHeight: 1.3 }}>Règlement Interne du Dispensaire</div>
              <div style={{ fontFamily: BODY, fontWeight: 400, fontSize: 15, color: '#596451' }}>Préambule et 10 chapitres — conduite, hygiène, soins, discipline</div>
            </div>
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontFamily: BODY, fontWeight: 400, fontSize: 15, color: '#596451' }}>Parcours de récolte des plantes médicinales du comté</div>
            </div>
          ...
          <div style={{ ...PANEL, overflow: 'hidden' }}>
            <div style={{ ...PANEL_HEAD, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => router.push('/redm/agenda')}
                style={{ fontFamily: MONO, fontSize: 12, padding: '5px 12px', cursor: 'pointer', background: 'rgba(128,104,45,0.12)', color: '#80682D', border: '1px solid rgba(128,104,45,0.35)', letterSpacing: '0.08em' }}>
                + AGENDA
              </button>
            </div>
            {rdvs.length === 0 ? (
              <div style={{ padding: '22px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontFamily: MONO, fontSize: 13, color: '#646850', letterSpacing: '0.08em' }}>Aucun rendez-vous à venir</span>
                <button onClick={() => router.push('/redm/agenda')}
                  style={{ fontFamily: MONO, fontSize: 12, padding: '5px 12px', cursor: 'pointer', background: 'transparent', color: '#6A6D50', border: '1px solid rgba(139,90,43,0.30)', letterSpacing: '0.06em' }}>
                  Planifier un RDV →
                </button>
              </div>
            ) : (
              <div>
                {rdvs.slice(0, 5).map((r, i) => {
                  ...
                  const statCol: Record<string, string> = { 'CONFIRMÉ': '#49654D', 'EN ATTENTE': '#80682D', 'ANNULÉ': '#8B4040', 'PASSÉ': '#646850' };
                  const col = isCabinet ? '#646850' : (statCol[r.statut] ?? '#6A6D50');
                  const isFirst = i === 0;
                  return (
                    <div key={r.id}
                      onClick={() => { if (isCabinet && !canAccessCabinet) return; router.push(isCabinet ? '/redm/cabinet/agenda' : '/redm/agenda'); }}
                      style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 22px', borderBottom: i < Math.min(rdvs.length, 5) - 1 ? '1px solid rgba(142,122,74,0.15)' : 'none', cursor: isCabinet && !canAccessCabinet ? 'default' : 'pointer', background: isFirst ? 'rgba(128,104,45,0.04)' : 'transparent', transition: 'background 0.15s', opacity: isCabinet ? 0.7 : 1 }}
                      onMouseEnter={e => (e.currentTarget.style.background = 'rgba(128,104,45,0.07)')}
                      onMouseLeave={e => (e.currentTarget.style.background = isFirst ? 'rgba(128,104,45,0.04)' : 'transparent')}>
                      <div style={{ textAlign: 'center', minWidth: 54, flexShrink: 0 }}>
                        <div style={{ fontFamily: MONO, fontSize: 18, color: isFirst && !isCabinet ? '#80682D' : '#6A6D50', lineHeight: 1 }}>{r.heure || '—'}</div>
                        <div style={{ fontFamily: MONO, fontSize: 11, color: '#646850', marginTop: 2, letterSpacing: '0.06em' }}>{r.date}</div>
                      </div>
                      <div style={{ width: 2, height: 32, background: `${col}60`, flexShrink: 0 }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: BODY, fontWeight: 600, fontSize: 16, color: isCabinet ? '#61654F' : '#243D45', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontStyle: isCabinet ? 'italic' : 'normal' }}>{r.patientNom}</div>
                        <div style={{ fontFamily: MONO, fontSize: 12, color: '#61654F', letterSpacing: '0.06em' }}>{isCabinet ? 'Cabinet Thérapeutique' : r.type}</div>
                      </div>
                      ...
                      {isCabinet && <span style={{ fontFamily: MONO, fontSize: 11, color: '#646850', background: 'rgba(90,74,53,0.15)', padding: '3px 8px', border: '1px solid rgba(90,74,53,0.25)', letterSpacing: '0.06em', flexShrink: 0 }}>🔒</span>}
                    </div>
                  );
                })}
                {rdvs.length > 5 && (
                  <div style={{ padding: '10px 22px', fontFamily: MONO, fontSize: 12, color: '#646850', letterSpacing: '0.08em' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontFamily: DISPLAY, fontSize: 19, color: '#244958', whiteSpace: 'nowrap', letterSpacing: '0.06em' }}>{TITRES.modules}</span>
            <div style={{ flex: 1, height: 1, background: 'rgba(142,122,74,0.50)' }} />
            <span style={{ fontFamily: BODY, fontWeight: 700, fontSize: 16, color: '#596451', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{visibleModules.length} Services</span>
          </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${h ? m.color + '70' : 'rgba(142,122,74,0.30)'}`,
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ fontFamily: BODY, fontWeight: 700, fontSize: 15, padding: '4px 10px', background: m.color + '18', color: m.color === '#80682D' ? '#80682D' : m.color, border: `1px solid ${m.color + '40'}`, letterSpacing: '0.10em', textTransform: 'uppercase' }}>
                      {m.badge}
                    </div>
                  </div>

                  <div>
                    <div style={{ fontFamily: DISPLAY, fontSize: 24, color: '#183746', lineHeight: 1.2, marginBottom: 5 }}>{m.label}</div>
                    <div style={{ fontFamily: BODY, fontWeight: 700, fontSize: 15, color: m.color === '#80682D' ? '#80682D' : m.color, letterSpacing: '0.12em', marginBottom: 8, textTransform: 'uppercase' }}>{m.sub}</div>
                    <div style={{ fontFamily: BODY, fontWeight: 400, fontSize: 18, color: '#46564E', lineHeight: 1.65 }}>{m.desc}</div>
                  </div>

                  <div style={{ paddingTop: 10, borderTop: `1px solid ${m.color + '25'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontFamily: BODY, fontWeight: 700, fontSize: 15, color: '#596451', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Disponible</span>
                    <span style={{ fontFamily: MONO, fontSize: 17, color: h ? (m.color === '#80682D' ? '#80682D' : m.color) : '#596451', transition: 'color 0.15s' }}>→ Consulter</span>
                  </div>
                </div>
              );
            })}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={PANEL}>
            ...
            <div style={{ padding: '16px 22px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontFamily: BODY, fontWeight: 400, fontSize: 20, color: '#344C4C' }}>{DISPENSAIRE.labelMedecins}</span>
                <span style={{ fontFamily: MONO, fontSize: 16, color: '#80682D', letterSpacing: '0.07em' }}>● {medecinsCount ?? '—'}</span>
              </div>

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                <span style={{ fontFamily: BODY, fontWeight: 400, fontSize: 20, color: '#344C4C' }}>{DISPENSAIRE.labelRisque}</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                  ...
                  {canAccessDirection && (
                    <button onClick={() => router.push('/redm/direction/alerte-sanitaire')}
                      style={{ fontFamily: MONO, fontSize: 11, padding: '4px 10px', cursor: 'pointer', background: 'rgba(128,104,45,0.12)', color: '#80682D', border: '1px solid rgba(128,104,45,0.35)', letterSpacing: '0.08em' }}>
                      GÉRER
                    </button>
                  )}
                </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                <span style={{ fontFamily: BODY, fontWeight: 400, fontSize: 20, color: '#344C4C' }}>{DISPENSAIRE.labelEpidemie}</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          {/* Citation */}
          <div style={{ ...PANEL, padding: '20px 24px', borderLeft: '3px solid rgba(142,122,74,0.50)' }}>
            <div style={{ fontFamily: BODY, fontWeight: 300, fontSize: 20, color: '#405247', lineHeight: 1.80, fontStyle: 'italic' }}>
              "{CITATION.text}"
            </div>
            <div style={{ fontFamily: MONO, fontSize: 15, color: '#71674D', marginTop: 12, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
              — {CITATION.author}
            </div>
          </div>
