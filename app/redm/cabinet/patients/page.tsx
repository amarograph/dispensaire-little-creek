...
const EQUILIBRES = [
  { val: '' as Equilibre,    label: '— Non évalué', col: '#5A4A35' },
  { val: 'I' as Equilibre,   label: 'Degré I — Léger',   col: '#4A6048' },
...
export default function PatientsPage() {
  ...
  return (
    <div style={{ fontFamily: BODY }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Rye&family=Josefin+Slab:wght@300;400;600;700&family=Special+Elite&display=swap'); @keyframes slide-in{from{transform:translateX(100%);opacity:0}to{transform:translateX(0);opacity:1}} @keyframes fade-in{from{opacity:0}to{opacity:1}}`}</style>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
        {[
          ...
          { l: 'EN COURS',   v: dossiers.filter(d=>d.statut==='EN COURS').length,  c: '#C8A850' },
          { l: 'TERMINÉES',  v: dossiers.filter(d=>d.statut==='TERMINÉE').length,  c: '#4A6048' },
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        ...
        <button onClick={openNew} style={{ fontFamily: MONO, fontSize: 14, letterSpacing: '0.1em', padding: '9px 20px', cursor: 'pointer', background: 'rgba(74,96,72,0.25)', color: '#8BAB88', border: '1px solid rgba(74,96,72,0.5)', whiteSpace: 'nowrap' }}>✚ NOUVEAU DOSSIER</button>
      </div>
                ...
                <div key={d.id} style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: `3px solid ${col}`, display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', cursor: 'pointer', transition: 'background 0.15s' }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontFamily: MONO, fontSize: 13, color: T.gold, background: 'rgba(200,168,80,0.10)', padding: '1px 7px' }}>{d.type}</span>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                    <button onClick={() => openEdit(d)} title="Modifier" style={{ fontFamily: MONO, fontSize: 14, padding: '5px 8px', cursor: 'pointer', background: 'rgba(200,168,80,0.10)', color: T.gold, border: `1px solid rgba(200,168,80,0.3)` }}>✎</button>
                    {archConfirm === d.id
                    ...
                    {delConfirm === d.id
                      ? <><button onClick={() => { deleteDossier(d.id); setDelConfirm(null); }} style={{ fontFamily: MONO, fontSize: 13, padding: '5px 8px', cursor: 'pointer', background: '#8B404025', color: '#C06060', border: '1px solid #8B404060' }}>OK?</button><button onClick={() => setDelConfirm(null)} style={{ fontFamily: MONO, fontSize: 13, padding: '5px 6px', cursor: 'pointer', background: 'transparent', color: T.dim, border: `1px solid ${T.border}` }}>✕</button></>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 540, background: T.bg, borderLeft: `2px solid ${T.gold}`, zIndex: 50, display: 'flex', flexDirection: 'column', animation: 'slide-in 0.25s ease' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 26px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                ...
                <label style={{ display:'flex', alignItems:'center', gap:6, cursor:'pointer', fontFamily:MONO, fontSize: 13, color: form.confidentiel?'#C06060':T.dim, paddingTop:16 }}>
            ...
            <div style={{ padding: '14px 26px 22px', borderTop: `1px solid ${T.border}`, flexShrink: 0 }}>
              <button onClick={submit} disabled={!form.patientNom.trim() || saving}
                style={{ width: '100%', fontFamily: MONO, fontSize: 15, letterSpacing: '0.12em', padding: '13px', cursor: form.patientNom.trim()?'pointer':'not-allowed', background: form.patientNom.trim()?'rgba(74,96,72,0.35)':'rgba(255,255,255,0.04)', color: form.patientNom.trim()?'#8BAB88':T.dim, border:`2px solid ${form.patientNom.trim()?'rgba(74,96,72,0.6)':'rgba(255,255,255,0.06)'}` }}>
