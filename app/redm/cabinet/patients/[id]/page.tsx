...
/* ── Constantes ── */
const STATUT_COL: Record<Statut, string> = { 'EN COURS': '#C8A850', 'TERMINÉE': '#4A6048', 'ABANDONNÉE': '#8B4040' };
const EQ_INFO: Record<string, { label: string; desc: string; col: string }> = {
  I:   { label: 'Degré I — Léger',   desc: "Troubles passagers, n'altérant point durablement les facultés du sujet", col: '#4A6048' },
...
const DOC_COL: Record<DocType, string> = {
  'Synthèse de consultation': '#C8A850', 'Prescription médicale': '#4A6048',
  'Rapport médical': '#486878', 'Lettre de correspondance': '#786030',
...
/* ── Constantes compta ── */
const PAI_COL:  Record<StatutPaiement, string> = { 'PAYÉ': '#4A6048', 'EN ATTENTE': '#C8A850', 'ANNULÉ': '#8B4040' };
...
const lbl: React.CSSProperties = { fontFamily: MONO, fontSize: 13, color: T.dim, letterSpacing: '0.12em', marginBottom: 6, display: 'block' };

function InfoLine({ label, value }: { label: string; value?: string }) {
  if (!value) return null;
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ fontFamily: MONO, fontSize: 13, color: T.dim, letterSpacing: '0.12em', marginBottom: 4 }}>{label}</div>
...
export default function PatientDetailPage() {
  ...
  return (
    <div style={{ fontFamily: BODY }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Rye&family=Josefin+Slab:wght@300;400;600;700&family=Special+Elite&display=swap'); @keyframes slide-in{from{transform:translateX(100%);opacity:0}to{transform:translateX(0);opacity:1}}`}</style>
        ...
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: `5px solid ${col}`, padding: '24px 28px', display: 'flex', alignItems: 'center', gap: 28, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8, flexWrap: 'wrap' }}>
              ...
              {dossier.confidentiel && <span style={{ fontFamily: MONO, fontSize: 13, color: '#C06060', background: '#8B404018', padding: '2px 8px' }}>CONFIDENTIEL</span>}
            </div>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ fontFamily: MONO, fontSize: 14, color: col, background: col + '18', padding: '3px 10px' }}>{dossier.statut}</span>
              <span style={{ fontFamily: MONO, fontSize: 14, color: T.gold, background: 'rgba(200,168,80,0.10)', padding: '3px 10px' }}>{dossier.type}</span>
          ...
          <div style={{ textAlign: 'center', flexShrink: 0 }}>
            ...
            <div style={{ fontFamily: MONO, fontSize: 13, color: T.dim, letterSpacing: '0.1em' }}>DOCUMENT{docs.length !== 1 ? 'S' : ''}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 22, borderBottom: `1px solid ${T.border}` }}>
        {([
          ...
          ['facturation', '💰 FACTURATION', '#C8A850', factures.length],
          ...
          return (
            <button key={key} onClick={() => setTab(key)}
              ...
              {label}
              {cnt !== null && cnt > 0 && <span style={{ fontFamily: MONO, fontSize: 12, color: col, background: `${col}22`, padding: '1px 6px' }}>{cnt}</span>}
            </button>
          );
        })}
      </div>
      ...
      {tab === 'dossier' && (
        <div>
          ...
          {(dossier.traitement || dossier.prochaine) && (
            <Block icon="💊" title="TRAITEMENT & RECOMMANDATIONS" color="#4A6048">
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
            <button onClick={openNewDoc}
              style={{ fontFamily: MONO, fontSize: 15, letterSpacing: '0.12em', padding: '10px 24px', cursor: 'pointer', background: 'rgba(200,168,80,0.15)', color: T.gold, border: `1px solid rgba(200,168,80,0.45)` }}>
              ✚ CRÉER UN DOCUMENT
            </button>
          </div>

          {docsError && (
            <div style={{ marginBottom: 16, padding: '12px 16px', background: 'rgba(200,80,80,0.10)', border: '1px solid rgba(200,80,80,0.35)', color: '#C06060', fontFamily: MONO, fontSize: 13, display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              }} style={{ fontFamily: MONO, fontSize: 12, padding: '3px 10px', cursor: 'pointer', background: 'rgba(200,80,80,0.15)', color: '#C06060', border: '1px solid rgba(200,80,80,0.4)' }}>
                ↺ Réessayer
              </button>
            </div>
          )}
                ...
                return (
                  <div key={doc.id} style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: `3px solid ${isOpen ? T.gold : T.border}`, overflow: 'hidden', transition: 'border-color 0.15s' }}>
                    ...
                    <div onClick={() => setExpandedDoc(isOpen ? null : doc.id)}
                      ...
                      <div style={{ width: 36, height: 36, background: isOpen ? 'rgba(200,168,80,0.18)' : 'rgba(200,168,80,0.08)', border: `1px solid rgba(200,168,80,${isOpen ? '0.5' : '0.2'})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: MONO, fontSize: 16, color: T.gold, flexShrink: 0, transition: 'all 0.15s' }}>
                        {String(idx + 1).padStart(2, '0')}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: DISPLAY, fontSize: 20, color: T.text }}>{doc.titre}</div>
                        <div style={{ fontFamily: MONO, fontSize: 13, color: T.dim, marginTop: 2 }}>{doc.date}{doc.contenu ? ` · ${doc.contenu.length} caractères` : ''}</div>
                      </div>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                        ...
                        <button onClick={() => openEditDoc(doc)}
                          style={{ fontFamily: MONO, fontSize: 14, padding: '5px 10px', cursor: 'pointer', background: 'rgba(200,168,80,0.10)', color: T.gold, border: `1px solid rgba(200,168,80,0.3)` }}>✎</button>
                        {delDocConfirm === doc.id
                          ? <><button onClick={() => deleteDoc(doc.id)} style={{ fontFamily: MONO, fontSize: 13, padding: '5px 8px', cursor: 'pointer', background: '#8B404025', color: '#C06060', border: '1px solid #8B404060' }}>OK?</button>
                    ...
                    <div key={f.id} style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: `4px solid ${col}` }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 18px' }}>
                        ...
                        <div style={{ textAlign: 'center', minWidth: 50, flexShrink: 0 }}>
                          ...
                          <div style={{ fontFamily: MONO, fontSize: 11, color: T.dim }}>{rpDisplay(f.dateSeance).slice(6)}</div>
                        </div>
                        {/* Prestations + payeur */}
                        <div style={{ flex: 1, display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                          {pres.map((p, i) => (
                            <span key={i} style={{ fontFamily: MONO, fontSize: 13, color: T.gold, background: 'rgba(200,168,80,0.10)', padding: '2px 8px', border: '1px solid rgba(200,168,80,0.20)' }}>
                              {p} <span style={{ color: T.muted }}>{fmt$(TARIFS[p] ?? 0)}</span>
                            </span>
                          ))}
                          <span style={{ fontFamily: MONO, fontSize: 12, color: '#88AAC0', background: 'rgba(72,104,120,0.18)', padding: '2px 7px' }}>{f.payeur}</span>
                        ...
                        {/* Statut */}
                        <div style={{ fontFamily: MONO, fontSize: 12, color: col, background: `${col}18`, padding: '5px 10px', border: `1px solid ${col}50`, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 600, background: T.bg, borderLeft: `2px solid ${T.gold}`, zIndex: 50, display: 'flex', flexDirection: 'column', animation: 'slide-in 0.25s ease' }}>
                ...
                <div style={{ display: 'flex', gap: 4, marginBottom: 6 }}>
                  {[
                    ...
                    <button key={label} type="button" title={title}
                      ...
                      style={{ fontFamily: BODY, fontSize: 18, ...style, padding: '4px 12px', cursor: 'pointer', background: 'rgba(200,168,80,0.08)', color: T.gold, border: `1px solid rgba(200,168,80,0.3)`, letterSpacing: 0 }}>
                      {label}
                    </button>
                  ))}
                  <span style={{ fontFamily: MONO, fontSize: 12, color: T.dim, alignSelf: 'center', marginLeft: 6 }}>Sélectionner du texte puis cliquer</span>
                </div>
                <textarea
                  ref={textareaRef}
            ...
            <div style={{ padding: '16px 28px 24px', borderTop: `1px solid ${T.border}`, flexShrink: 0 }}>
              {docError && (
                <div style={{ marginBottom: 10, padding: '8px 14px', background: 'rgba(200,80,80,0.12)', border: '1px solid rgba(200,80,80,0.35)', color: '#C06060', fontFamily: MONO, fontSize: 13 }}>
                  ⚠ {docError}
                </div>
              )}
              <button onClick={submitDoc} disabled={!docTitre.trim() || docSaving}
                style={{ width: '100%', fontFamily: MONO, fontSize: 16, letterSpacing: '0.14em', padding: '14px', cursor: docTitre.trim() && !docSaving ? 'pointer' : 'not-allowed', background: docTitre.trim() ? 'rgba(200,168,80,0.20)' : 'rgba(255,255,255,0.04)', color: docTitre.trim() ? T.gold : T.dim, border: `2px solid ${docTitre.trim() ? 'rgba(200,168,80,0.55)' : 'rgba(255,255,255,0.06)'}` }}>
