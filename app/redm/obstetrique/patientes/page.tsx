...
const RISQUES = [
  { val: '' as Risque,    label: '— Non évalué',           col: '#C8BEA5' },
  { val: 'I' as Risque,   label: 'Degré I — Faible',        col: '#A8B991' },
  { val: 'II' as Risque,  label: 'Degré II — Modéré',       col: '#786030' },
  { val: 'III' as Risque, label: 'Degré III — Élevé',       col: '#8B4040' },
];
...
export default function PatientesPage() {
  ...
  useEffect(() => {
    fetch('/api/obstetrique/dossiers')
      .then(r => r.json())
      .then(d => { setDossiers(d.dossiers ?? []); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);

  function openNew() { router.push('/redm/obstetrique/patientes/nouveau'); }
  ...
  function openEdit(d: Dossier) {
    ...
    setForm({
      ...
      antecedentsObstetricaux: d.antecedentsObstetricaux ?? '',
      antecedentsFamiliaux:    d.antecedentsFamiliaux    ?? '',
      evenementsRecents:       d.evenementsRecents       ?? '',
      risqueGrossesse:         d.risqueGrossesse         ?? '',
      noteObstetricien:        d.noteObstetricien        ?? '',
  ...
  async function submit() {
    ...
    setSaving(true);
    if (editing) {
      await fetch('/api/obstetrique/dossiers', {
        method: 'PATCH',
    ...
    <div style={{ fontFamily: BODY }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
          <button onClick={() => router.push('/redm/obstetrique')} style={{ fontFamily: MONO, fontSize: 15, background: 'transparent', border: `1px solid ${T.border}`, color: T.muted, padding: '8px 18px', cursor: 'pointer', letterSpacing: '0.1em' }}>← RETOUR</button>
          <span style={{ fontFamily: MONO, fontSize: 14, color: T.gold, letterSpacing: '0.16em' }}>OBSTÉTRIQUE · DOSSIERS PATIENTES</span>
        </div>
        <h1 style={{ fontFamily: DISPLAY, fontSize: 35, color: T.gold, margin: 0 }}>📋 Dossiers Patientes</h1>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
        {[
          ...
          { l: 'ACCOUCHÉES', v: dossiers.filter(d=>d.statut==='ACCOUCHÉE').length, c: '#A8B991' },
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher une patiente…" style={{ ...inp, flex: 1 }} />
        <button onClick={() => router.push('/redm/obstetrique/archives/cloturees')}
      ...
      : filtered.length === 0
        ? <div style={{ background: T.card, border: `1px solid ${T.border}`, padding: '40px', textAlign: 'center', fontFamily: MONO, fontSize: 15, color: T.dim }}>{dossiers.length === 0 ? 'Aucun dossier patiente' : 'Aucun résultat'}</div>
            ...
            {filtered.map(d => {
              ...
              return (
                <div key={d.id} style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: `3px solid ${col}`, display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', cursor: 'pointer', transition: 'background 0.15s' }}
                  onClick={() => router.push('/redm/obstetrique/patientes/' + d.id)}
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 540, background: T.bg, borderLeft: `2px solid ${T.gold}`, zIndex: 50, display: 'flex', flexDirection: 'column', animation: 'slide-in 0.25s ease' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 26px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div><label style={lbl}>MOTIF / TERME DE GROSSESSE</label><input style={inp} value={form.plainte} onChange={e => setForm(f=>({...f,plainte:e.target.value}))} placeholder="Motif de la visite, semaines de grossesse…" /></div>

              {/* INFORMATIONS */}
              <div style={{ ...sec, marginTop: 8 }}>📜 INFORMATIONS</div>
              <div><label style={lbl}>ANTÉCÉDENTS OBSTÉTRICAUX</label><textarea style={{...inp,resize:'vertical',minHeight:70,lineHeight:1.6}} value={form.antecedentsObstetricaux} onChange={e => setForm(f=>({...f,antecedentsObstetricaux:e.target.value}))} placeholder="Grossesses, accouchements et fausses couches passés…" /></div>
              ...
              <div><label style={lbl}>ÉVÉNEMENTS RÉCENTS / COMPLICATIONS</label><textarea style={{...inp,resize:'vertical',minHeight:60,lineHeight:1.6}} value={form.evenementsRecents} onChange={e => setForm(f=>({...f,evenementsRecents:e.target.value}))} placeholder="Saignements, douleurs, chocs…" /></div>

              {/* RISQUE DE GROSSESSE */}
              <div style={{ ...sec, marginTop: 8 }}>⚖ NIVEAU DE RISQUE DE LA GROSSESSE</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {RISQUES.map(r => {
                  const on = form.risqueGrossesse === r.val;
                  return (
                    <button key={r.val} type="button" onClick={() => setForm(f=>({...f,risqueGrossesse:r.val}))}
                      style={{ fontFamily: MONO, fontSize: 14, padding: '9px 12px', cursor: 'pointer', background: on ? r.col+'28' : 'transparent', color: on ? r.col : T.dim, border: `1px solid ${on ? r.col+'70' : T.border}`, textAlign: 'left', letterSpacing: '0.06em' }}>
                      {r.label}
                    </button>
                  );
                })}
              ...
              {/* NOTE OBSTÉTRICIEN */}
              <div style={{ ...sec, marginTop: 8 }}>✍ NOTE DE L&apos;OBSTÉTRICIEN</div>
              <div><textarea style={{...inp,resize:'vertical',minHeight:80,lineHeight:1.7}} value={form.noteObstetricien} onChange={e => setForm(f=>({...f,noteObstetricien:e.target.value}))} placeholder="Impressions cliniques, observations…" /></div>
              ...
              <div><label style={lbl}>PROCHAINE CONSULTATION</label><input style={inp} value={form.prochaine} onChange={e => setForm(f=>({...f,prochaine:e.target.value}))} placeholder="Date ou note de suivi" /></div>
              <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                <div style={{ flex: 1 }}><label style={lbl}>STATUT</label><select style={{...inp,cursor:'pointer'}} value={form.statut} onChange={e => setForm(f=>({...f,statut:e.target.value as Statut}))}><option>EN COURS</option><option>ACCOUCHÉE</option><option>ABANDONNÉE</option><option>CLÔTURÉE</option></select></div>
